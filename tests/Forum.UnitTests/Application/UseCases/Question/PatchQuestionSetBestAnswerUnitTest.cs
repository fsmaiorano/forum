...
public class PatchQuestionSetBestAnswerUnitTest(TestFixture fixture) : BaseTest(fixture)
{
    ...
    [Fact]
    public async Task PatchQuestionSetBestAnswer_ShouldRaiseDomainEvent()
    {
        ClearDomainEvents();
        ...
        Assert.True(dispatcher.HasEvent<QuestionBestAnswerChosenEvent>(),
            "QuestionBestAnswerChosenEvent should have been dispatched");

        var domainEvent = dispatcher.GetEvent<QuestionBestAnswerChosenEvent>();

        Assert.NotNull(updatedQuestion);
        Assert.NotNull(updatedQuestion.BestAnswerId);
        Assert.NotNull(domainEvent);
        Assert.Equal(updatedQuestion.Id, domainEvent.GetAggregateId());
    }
}